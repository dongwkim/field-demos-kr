...
# MAGIC 
# MAGIC In our case, the audit log has been enabled and is being delivered under this cloud storage: `s3a://databricks-field-eng-audit-logs/e2-demo-field-eng`
# MAGIC 
...
# MAGIC CREATE DATABASE IF NOT EXISTS demos.audit_log;
...
def ingest_audit_log():
  ...
  (streamDF.writeStream
            ...
            .toTable('demos.audit_log.bronze').awaitTermination())
...
# MAGIC %sql select distinct(workspaceId) from demos.audit_log.bronze order by workspaceId

# COMMAND ----------

# DBTITLE 1,Audit log is enabled for all services, including "unityCatalog" which is what we're interested in
# MAGIC %sql select distinct(serviceName) from demos.audit_log.bronze order by serviceName desc
...
def flatten_table(service_name):
  ...
  flattenedStream = spark.readStream.table("demos.audit_log.bronze")
  ...
  (flattenedStream
   ...
   .writeStream
     ...
     .trigger(availableNow=True)
     .option("mergeSchema", True)
     .option("checkpointLocation", f"/demo/uc/audit_log/gold/{service_name}/checkpoint")
     .table(f"demos.audit_log.{table_name}").awaitTermination())


# COMMAND ----------

service_name_list = [i['serviceName'] for i in spark.table('demos.audit_log.bronze').select("serviceName").distinct().collect() if i['serviceName'] is not None]
...
# MAGIC %sql SELECT * except(email, sourceIPAddress), sha1(email) as email FROM demos.audit_log.unitycatalog 
...
def maintain_table(service_name):
  ...
  spark.sql(f"GRANT ALL PRIVILEGES ON TABLE demos.audit_log.{table_name} TO `account users`")
  spark.sql(f"ALTER TABLE demos.audit_log.{table_name} SET TBLPROPERTIES (delta.autoOptimize.optimizeWrite = true, delta.autoOptimize.autoCompact = true)")
  spark.sql(f"VACUUM demos.audit_log.{table_name} retain 200 hours")
