...
def generate_sensor_data(sensor_conf, faulty = False, sample_size = 1000, display_graph = True, noise = 2, delta = -3):
  dg = sdg()
  dg.seed(42)
...
for turbine in range(turbine_count):
  ...
  for s in sensors:
    ...
    plot = turbine == 0
    df['sensor_'+s['name']] = generate_sensor_data(s, damaged_sensor, sample_size, plot)
...
spark_df = spark.createDataFrame(final_df).cache()
...
"""first_id = final_df['turbine_id'].iloc[0]
t1 = final_df[final_df['turbine_id'] == first_id]
t1['D']"""
...
t1['datetime'] = pd.to_datetime(t1['timestamp'], unit='s')
