...
if len(catalog) > 0:
  ...
  try:
    catalogs = [r['catalog'] for r in spark.sql("SHOW CATALOGS").collect()]
    if len(catalogs) == 1 and catalogs[0] == 'spark_catalog':
      print(f"UC doesn't appear to be enabled, will fallback to hive_metastore")
...
print(f"using catalog.databse {catalog}.{dbName}")
