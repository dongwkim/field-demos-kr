...
# MAGIC <img width="1px" src="https://www.google-analytics.com/collect?v=1&gtm=GTM-NKQ8TT7&tid=UA-163989034-1&cid=555&aip=1&t=event&ec=field_demos&ea=display&dp=%2F42_field_demos%2Fhls%2Fomop%2Fanalysis%2Fchf&dt=HLS_OMOP_CHF">
...
# MAGIC %sql
# MAGIC select *, 1000*events/days/7 as ir from INCIDENT_RATE
# MAGIC -- plot option: keys=days, group=gender, values=ir

# COMMAND ----------

library(ggplot2)    

data_df <- sql("select *, 1000*events/days/7 as ir from INCIDENT_RATE")
data_rdf = collect(data_df)

ggplot(data_rdf, aes(x=days, y=ir, group=gender, colour=gender ) ) + geom_line(size=1) + theme(aspect.ratio=8/10)
...
# MAGIC   INSERT INTO cohort_definition
# MAGIC   select
...
# MAGIC %md-sandbox
# MAGIC 
# MAGIC ## 5. Building and sharing visualization
# MAGIC 
# MAGIC <img src="https://github.com/QuentinAmbard/databricks-demo/raw/main/hls/resources/dbinterop/hls-patient-dashboard.png" width="700" style="float: right" />
# MAGIC 
# MAGIC Ultimately, we can leverage Databricks SQL capabilities to build interactive dashboard and share analysis. 
# MAGIC 
