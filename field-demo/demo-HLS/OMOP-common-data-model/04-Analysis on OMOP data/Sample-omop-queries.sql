...
-- MAGIC 
-- MAGIC <img width="1px" src="https://www.google-analytics.com/collect?v=1&gtm=GTM-NKQ8TT7&tid=UA-163989034-1&cid=555&aip=1&t=event&ec=field_demos&ea=display&dp=%2F42_field_demos%2Fhls%2Fomop%2Fanalysis%2Fomop_queries&dt=HLS_OMOP_QUERIES">
...
-- MAGIC %python spark.sql(f"DESCRIBE DATABASE {dbName}").display()

-- COMMAND ----------

select * from source_to_standard_vocab_map
...
SELECT NVL(state, 'XX' ) AS state_abbr, count(*) as Num_Persons_count
  FROM person
