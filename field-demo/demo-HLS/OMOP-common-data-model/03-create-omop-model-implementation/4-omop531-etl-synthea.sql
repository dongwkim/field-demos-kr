...
-- MAGIC 
-- MAGIC <img src="https://github.com/QuentinAmbard/databricks-demo/raw/main/hls/resources/hls-omop-flow-4.png" width="1000" >
-- MAGIC 
-- MAGIC 
-- MAGIC We have now loaded Synthea data as Delta Tables. 
-- MAGIC 
-- MAGIC The next step is to Transform and Load this data to the OMOP model created in the first step. Note that ETL process is heavily dependent on the source data model which varies from institution to institution. Arguably writing the ETL is the most time consuming part of the process.
-- MAGIC 
-- MAGIC In this notebook, we use an example ETL, based on [OHDSI/ETL-Synthea](https://github.com/OHDSI/ETL-Synthea) repository, for mapping from synthea model to OMOP to populate a subset of OMOP tables as a point of reference for benchmarking. 
-- MAGIC 
-- MAGIC Also, we provide example notebooks for data analysis on the resulting omop tables using SQL, python and R. 
-- MAGIC 
-- MAGIC <!-- Collect usage data (view). Remove it to disable collection. View README for more details.  -->
-- MAGIC <img width="1px" src="https://www.google-analytics.com/collect?v=1&gtm=GTM-NKQ8TT7&tid=UA-163989034-1&cid=555&aip=1&t=event&ec=field_demos&ea=display&dp=%2F42_field_demos%2Fhls%2Fomop%2Fbuild_model%2Fnotebook_04&dt=HLS_OMOP">
...
CREATE TABLE IF NOT EXISTS all_visits
SELECT
  *,
  ROW_NUMBER() OVER(
    ORDER BY
...
CREATE TABLE IF NOT EXISTS assign_all_visit_ids
SELECT
...
CREATE TABLE IF NOT EXISTS final_visit_ids
