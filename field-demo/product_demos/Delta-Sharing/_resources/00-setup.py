...
if "airlinedata" not in [r.name for r in spark.catalog.listDatabases()]:
    spark.sql("""CREATE SCHEMA IF NOT EXISTS airlinedata COMMENT "all flight data recieved from partnered airlines and augmented with tripactions platform" """)
    spark.sql("""GRANT USAGE, CREATE ON SCHEMA airlinedata TO `account users`""")

# COMMAND ----------

def table_exists(tb):
    #TODO: spark.catalog.tableExists not available?
    return tb in [t.name for t in spark.catalog.listTables("airlinedata")]
...
def init_data():
    ...
    if not table_exists("flights"):
        ...
        airlineDataTemp.write.partitionBy("Year", "UniqueCarrier").mode('overwrite').saveAsTable("airlinedata.flights")
        spark.sql("""GRANT SELECT, MODIFY ON TABLE airlinedata.flights TO `account users`""")
        spark.sql("""ALTER TABLE airlinedata.flights OWNER TO `account users`""")
    ...
    if not table_exists("lookupcodes"):
        ...
        df_codes.write.mode('overwrite').saveAsTable("airlinedata.lookupcodes")
        spark.sql("""GRANT SELECT, MODIFY ON TABLE airlinedata.lookupcodes TO `account users`""")
        spark.sql("""ALTER TABLE airlinedata.flights OWNER TO `account users`""")
        spark.sql("OPTIMIZE airlinedata.lookupcodes")
    ...
    if not table_exists("airports"):
        ...
        airportsna.write.mode('overwrite').saveAsTable("airlinedata.airports")
        spark.sql("""GRANT SELECT, MODIFY ON TABLE airlinedata.airports TO `account users`""")
        spark.sql("""ALTER TABLE airlinedata.airports OWNER TO `account users`""")
        spark.sql("OPTIMIZE airlinedata.airports")
