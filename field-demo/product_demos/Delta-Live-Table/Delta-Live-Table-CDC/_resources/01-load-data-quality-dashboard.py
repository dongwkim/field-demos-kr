...
spark.createDataFrame(expectations, headers.split(",")) \
  ...
  .withColumn("dropped_records", F.col("dropped_records").cast("int")).write.mode("overwrite").option("overwriteSchema", "true").saveAsTable("demos.dlt_expectations")

# COMMAND ----------

# MAGIC %sql
# MAGIC alter table demos.dlt_expectations set tblproperties (delta.autoOptimize.optimizeWrite = true, delta.autoOptimize.autoCompact = true)
