...
def display_automl_churn_link(force_refresh = False): 
  if force_refresh:
    reset_automl_run("churn_auto_ml")
  display_automl_link("churn_auto_ml", "demos_customer_churn", spark.table("churn_features"), "churn", 5)

def get_automl_churn_run(force_refresh = False): 
  if force_refresh:
    reset_automl_run("churn_auto_ml")
  from_cache, r = get_automl_run_or_start("churn_auto_ml", "demos_customer_churn", spark.table("churn_features"), "churn", 5)
  return r
