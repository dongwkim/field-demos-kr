...
client.set_tag(run_id, key='db_table', value=f'{dbName}.churn_features')

#Deploy our autoML run in MLFlow registry
model_details = mlflow.register_model(f"runs:/{run_id}/model", "demos_customer_churn")
...
model_version_details = client.get_model_version(name="demos_customer_churn", version=model_details.version)
...
request_transition(model_name = "demos_customer_churn", version = model_details.version, stage = "Staging")
...
model_comment(model_name = "demos_customer_churn",
             version = model_details.version,
             comment = comment)
